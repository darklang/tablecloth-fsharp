...
let first t =
    if length t < 1 then None else Some t[0]

let last t =
    if length t < 1 then None else Some t[length t - 1]

let get i a = Array.get a i

let getAt index a =
    if index >= 0 && index < length a then
        Some(Array.get a index)
    else
        None
...
let foldRight initial f a =
    Array.foldBack (fun v accum -> f accum v) a initial
...
let zip a b =
    map2 (fun left right -> (left, right)) a b

let map3 (f: 'a -> 'b -> 'c -> 'd) (arrayA: 'a array) (arrayB: 'b array) (arrayC: 'c array) =
...
let intersperse sep array =
    initialize (fun i -> if i % 2 <> 0 then sep else get (i / 2) array) (max 0 ((Array.length array * 2) - 1))
...
let sliding step size a =
    ...
    if size > n then
        ...
        initialize (fun i -> initialize (fun j -> a[(i * step) + j]) size) (1 + ((n - size) / step))
...
let maximum a =
    if length a = 0 then None else Some(Array.max a)

let minimum a =
    if length a = 0 then None else Some(Array.min a)
    ...
    if length a <> length b then
        ...
        let rec loop index =
            if index = length a then
                ...
                f a[index] b[index] && loop (index + 1)
